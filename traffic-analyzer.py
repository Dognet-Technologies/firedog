...
class TrafficAnalyzer:
    ...
    def generate_report(self):
        ...
        print("\n\n🎯 TOP 20 IP SORGENTI per THREAT SCORE")
        print("-" * 70)
        ...
        print(f"🔴 CRITICHE (score >= 80):  {len(threats['critical']):>3}")
        print(f"🟠 ALTE     (score >= 60):  {len(threats['high']):>3}")
        print(f"🟡 MEDIE    (score >= 40):  {len(threats['medium']):>3}")
        print(f"🟢 BASSE    (score >= 20):  {len(threats['low']):>3}")
        
        # Dettagli minacce critiche
        if threats['critical']:
            print("\n\n🔴 DETTAGLIO MINACCE CRITICHE")
            print("-" * 70)
    ...
    def generate_recommendations(self, threats):
        ...
        if threats['high'] or threats['critical']:
            ...
            print("   - Implementare rate limiting più aggressivo")
        
        if threats['medium']:
            print("\n🟡 MONITORAGGIO:")
            print("   - Continuare osservazione pattern IP a medio rischio")
            print("   - Verificare se traffico è legittimo (crawlers, monitoring)")

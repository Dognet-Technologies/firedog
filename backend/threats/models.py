...
class ThreatLog(models.Model):
    ...
    protocol = models.CharField(
        max_length=10, blank=True, help_text="Protocollo utilizzato"
    )
    ...
    reasons = models.JSONField(
        default=list, help_text="Motivi del threat score (lista di stringhe)"
    )
    ...
    country_code = models.CharField(
        max_length=2, blank=True, help_text="Codice paese ISO dell'IP sorgente"
    )

    # Stato
    is_blocked = models.BooleanField(
        default=False, db_index=True, help_text="Indica se l'IP è stato bloccato"
    )
    ...
    resolved_at = models.DateTimeField(
        null=True, blank=True, help_text="Quando la minaccia è stata risolta"
    )
    ...
    def __str__(self):
        return (
            f"{self.source_ip} -> {self.target.ip_address} (score: {self.threat_score})"
        )

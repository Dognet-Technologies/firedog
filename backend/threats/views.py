...
from .serializers import ThreatLogSerializer, ThreatLogListSerializer, ThreatLogStatsSerializer
...
class ThreatLogViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        stats = {
            ...
            'blocked_ips': queryset.filter(is_blocked=True).values('source_ip').distinct().count(),
            'resolved_threats': queryset.filter(is_resolved=True).count(),
            'unresolved_threats': queryset.filter(is_resolved=False).count(),
            'top_attackers': list(queryset.values('source_ip').annotate(
                count=Count('id')
            ).order_by('-count')[:10]),
            'recent_threats': ThreatLogListSerializer(queryset.order_by('-detected_at')[:20], many=True).data
        }

...
class Dashboard(models.Model):
    ...
    name = models.CharField(
        max_length=255,
        help_text="Nome della dashboard"
    )

    description = models.TextField(
        blank=True,
        help_text="Descrizione della dashboard"
    )

    # Configurazione
    is_default = models.BooleanField(
        default=False,
        help_text="Dashboard di default per l'utente"
    )

    is_public = models.BooleanField(
        default=False,
        help_text="Dashboard visibile ad altri utenti"
    )
...
class Widget(models.Model):
    ...
    grid_position = models.JSONField(
        default=dict,
        help_text="Posizione nel grid layout {x, y, w, h}"
    )

    # Stato
    is_visible = models.BooleanField(
        default=True,
        help_text="Widget visibile nella dashboard"
    )

...
class PairingSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'target', 'target_hostname', 'target_ip', 'status',
            'phase_1_verified', 'phase_2_verified',
            'agent_ip', 'agent_hostname', 'agent_mac',
            'error_message', 'created_at', 'expires_at', 'completed_at'
        ]
        read_only_fields = [
            'id', 'status', 'phase_1_verified', 'phase_2_verified',
            'agent_ip', 'agent_hostname', 'agent_mac',
            'error_message', 'created_at', 'expires_at', 'completed_at'
        ]
...
class AgentConnectionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'websocket_channel', 'is_online',
            'last_heartbeat', 'heartbeat_interval',
            'system_info', 'connected_at', 'disconnected_at'
        ]
        read_only_fields = [
            'websocket_channel', 'is_online', 'last_heartbeat',
            'system_info', 'connected_at', 'disconnected_at'
        ]
...
class AgentCommandSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AgentCommand
        fields = [
            'command_id', 'target', 'target_hostname', 'target_ip',
            'action', 'payload', 'status',
            'result', 'error_message', 'timeout_seconds',
...
class AgentHeartbeatSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'target', 'target_hostname', 'target_ip',
            'timestamp', 'cpu_percent', 'memory_percent', 'disk_percent',
            'bytes_sent', 'bytes_recv',
            'active_rules_count', 'blocked_ips_count',
            'raw_data'
        ]

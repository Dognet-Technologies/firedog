...
class PairingSessionAdmin(admin.ModelAdmin):
    list_display = ['id', 'target', 'status', 'phase_1_verified', 'phase_2_verified', 'created_at', 'expires_at']
    list_filter = ['status', 'phase_1_verified', 'phase_2_verified', 'created_at']
    search_fields = ['target__hostname', 'target__ip_address', 'agent_ip', 'agent_hostname']
...
class AgentCommandAdmin(admin.ModelAdmin):
    list_display = ['command_id', 'target', 'action', 'status', 'created_at', 'completed_at']
...
class AgentHeartbeatAdmin(admin.ModelAdmin):
    list_display = ['id', 'target', 'timestamp', 'cpu_percent', 'memory_percent', 'disk_percent']

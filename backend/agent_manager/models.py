...
class AgentAPIKey(models.Model):
    ...
    key_hash = models.CharField(
        max_length=128, unique=True, help_text="SHA512 hash dell'API key"
    )
    ...
    is_active = models.BooleanField(
        default=True, db_index=True, help_text="Solo una chiave può essere attiva"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="Scadenza chiave (null = mai)"
    )
    created_by = models.CharField(
        max_length=100, help_text="Utente che ha creato la chiave"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_active:
            AgentAPIKey.objects.filter(is_active=True).exclude(pk=self.pk).update(
                is_active=False
            )
...
class PairingSession(models.Model):
    ...
    phase_1_verified = models.BooleanField(
        default=False, help_text="API key verificata"
    )
    phase_2_verified = models.BooleanField(
        default=False, help_text="Identity hash verificato"
    )

    # Dati agent ricevuti
    agent_ip = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP ricevuto dall'agent"
    )
    agent_hostname = models.CharField(
        max_length=255, blank=True, help_text="Hostname ricevuto dall'agent"
    )
    agent_mac = models.CharField(
        max_length=17, blank=True, help_text="MAC address ricevuto dall'agent"
    )

    # Messaggi errore
    error_message = models.TextField(
        blank=True, help_text="Messaggio di errore in caso di fallimento"
    )
    ...
    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="Quando il pairing è stato completato"
    )
...
class AgentConnection(models.Model):
    ...
    websocket_channel = models.CharField(
        max_length=255, help_text="Django Channels channel name"
    )

    is_online = models.BooleanField(
        default=True, db_index=True, help_text="Agent connesso e attivo"
    )

    # Heartbeat
    last_heartbeat = models.DateTimeField(
        auto_now_add=True, db_index=True, help_text="Ultimo heartbeat ricevuto"
    )
    heartbeat_interval = models.PositiveIntegerField(
        default=30, help_text="Intervallo heartbeat in secondi"
    )

    # System info (ultimo ricevuto)
    system_info = models.JSONField(
        null=True, blank=True, help_text="Statistiche sistema dall'ultimo heartbeat"
    )

    # Metadata
    connected_at = models.DateTimeField(auto_now_add=True)
    disconnected_at = models.DateTimeField(
        null=True, blank=True, help_text="Quando l'agent si è disconnesso"
    )
...
class AgentCommand(models.Model):
    ...
    action = models.CharField(
        max_length=50, choices=ACTION_CHOICES, help_text="Azione da eseguire"
    )
    ...
    result = models.JSONField(
        null=True, blank=True, help_text="Risultato esecuzione comando"
    )
    error_message = models.TextField(blank=True, help_text="Messaggio di errore")

    # Timeout
    timeout_seconds = models.PositiveIntegerField(
        default=30, help_text="Timeout comando in secondi"
    )

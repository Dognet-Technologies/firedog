...
class SystemSettingsBulkSerializer(serializers.Serializer):
    ...
    settings = serializers.DictField(
        ...
        help_text="Dizionario di impostazioni da salvare"
    )
...
class SSHKeySerializer(serializers.ModelSerializer):
    ...
    associated_targets = serializers.IntegerField(
        source='associated_targets_count',
        read_only=True
    )
...
class SSHKeyImportSerializer(serializers.Serializer):
    ...
    private_key = serializers.CharField(
        write_only=True,
        help_text="Chiave privata SSH (formato OpenSSH)"
    )
    ...
    def validate_private_key(self, value):
        ...
        if not value.startswith('-----BEGIN'):
            raise serializers.ValidationError(
                "Formato chiave privata non valido. "
                "Deve iniziare con -----BEGIN"
            )
...
class DatabaseStatsSerializer(serializers.Serializer):
    ...
    tables_size = serializers.DictField(
        ...
        help_text="Dimensione per tabella"
    )
...
class DatabaseCleanupSerializer(serializers.Serializer):
    ...
    dry_run = serializers.BooleanField(
        default=False,
        help_text="Se True, simula l'operazione senza eliminare"
    )
...
class DatabaseCleanupLogSerializer(serializers.ModelSerializer):
    ...
    cleanup_type_display = serializers.CharField(
        source='get_cleanup_type_display',
        read_only=True
    )
...
class NotificationConfigSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'smtp_password': {'write_only': True}  # Non esporre in lettura
        }
...
class NotificationTestSerializer(serializers.Serializer):
    ...
    notification_type = serializers.ChoiceField(
        choices=['email', 'slack', 'discord'],
        help_text="Tipo di notifica da testare"
    )
...
class ChangePasswordSerializer(serializers.Serializer):
    ...
    current_password = serializers.CharField(
        write_only=True,
        help_text="Password attuale"
    )
    
    new_password = serializers.CharField(
        write_only=True,
        min_length=9,
        help_text="Nuova password"
    )
    
    confirm_password = serializers.CharField(
        write_only=True,
        help_text="Conferma nuova password"
    )

...
class FileIntegrity(models.Model):
    ...
    sha512_hash = models.CharField(
        max_length=128,
        help_text="Hash SHA512 del contenuto del file"
    )
    ...
    file_owner = models.CharField(
        max_length=100,
        blank=True,
        help_text="Proprietario del file"
    )
    ...
    last_checked = models.DateTimeField(
        auto_now=True,
        help_text="Ultimo controllo effettuato"
    )
    ...
    change_notes = models.TextField(
        blank=True,
        help_text="Note sulla modifica"
    )

    # Alert
    alert_sent = models.BooleanField(
        default=False,
        help_text="Alert inviato per questa modifica"
    )

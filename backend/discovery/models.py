...
class DiscoveredHost(models.Model):
    ...
    mac_address = models.CharField(
        max_length=17, blank=True, help_text="Indirizzo MAC (formato XX:XX:XX:XX:XX:XX)"
    )

    hostname = models.CharField(
        max_length=255, blank=True, help_text="Hostname risolto (se disponibile)"
    )

    # Vendor info (da MAC lookup)
    vendor = models.CharField(
        max_length=255, blank=True, help_text="Vendor dell'interfaccia di rete"
    )

    # Network info
    network = models.GenericIPAddressField(
        null=True, blank=True, help_text="Rete di appartenenza"
    )

    netmask = models.CharField(
        max_length=15, blank=True, help_text="Netmask della rete"
    )

    # Discovery info
    discovered_at = models.DateTimeField(
        auto_now_add=True, db_index=True, help_text="Quando l'host è stato scoperto"
    )

    last_seen = models.DateTimeField(
        auto_now=True, help_text="Ultimo scan in cui l'host è stato visto"
    )

    scan_count = models.PositiveIntegerField(
        default=1, help_text="Numero di volte che l'host è stato visto negli scan"
    )

    # Stato
    is_alive = models.BooleanField(
        default=True, db_index=True, help_text="Host ancora attivo nell'ultimo scan"
    )

    is_imported = models.BooleanField(
        default=False, db_index=True, help_text="Host importato come target"
    )

    # Note
    notes = models.TextField(blank=True, help_text="Note sull'host scoperto")

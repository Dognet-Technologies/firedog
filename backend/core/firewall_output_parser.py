...
class FirewallOutputParser:
    ...
    def parse_stats(cls, output: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Parse INPUT packets
            input_match = re.search(r'INPUT:\s+(\d+)\s+pacchetti\s+\((\d+(?:\.\d+)?)\s*([KMG]?B)\)', clean_output)
            if input_match:
                stats['input_packets'] = int(input_match.group(1))
                stats['input_bytes'] = cls._convert_size_to_bytes(float(input_match.group(2)), input_match.group(3))
            
            # Parse INPUT dropped
            input_dropped_match = re.search(r'INPUT:.*?Dropped:\s*(\d+)\s+\((\d+(?:\.\d+)?)%\)', clean_output, re.DOTALL)
            ...
            output_match = re.search(r'OUTPUT:\s+(\d+)\s+pacchetti\s+\((\d+(?:\.\d+)?)\s*([KMG]?B)\)', clean_output)
            if output_match:
                stats['output_packets'] = int(output_match.group(1))
                stats['output_bytes'] = cls._convert_size_to_bytes(float(output_match.group(2)), output_match.group(3))
            
            # Parse OUTPUT dropped
            output_dropped_match = re.search(r'OUTPUT:.*?Dropped:\s*(\d+)\s+\((\d+(?:\.\d+)?)%\)', clean_output, re.DOTALL)
            ...
            pcap_input_match = re.search(r'INPUT:\s+(\d+(?:\.\d+)?)\s*([KMG]?B)\s+\(/var/log/ulogd/input', clean_output)
            if pcap_input_match:
                stats['pcap_input_size'] = cls._convert_size_to_bytes(float(pcap_input_match.group(1)), pcap_input_match.group(2))
            
            # Parse PCAP OUTPUT size
            pcap_output_match = re.search(r'OUTPUT:\s+(\d+(?:\.\d+)?)\s*([KMG]?B)\s+\(/var/log/ulogd/output', clean_output)
            if pcap_output_match:
                stats['pcap_output_size'] = cls._convert_size_to_bytes(float(pcap_output_match.group(1)), pcap_output_match.group(2))
    ...
    def parse_rules(cls, output: str, chain: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            chain_sections = re.finditer(r'Chain\s+(\w+)\s+\(policy\s+(\w+)', clean_output)
            ...
            for chain_match in chain_sections:
                ...
                chain_end = chain_start + next_chain.start() if next_chain else len(clean_output)
                ...
                for line in chain_section.split('\n'):
                    ...
                    if rule_match:
                        ...
                        extra = rule_match.group(8).strip() if rule_match.group(8) else ''
            ...
            for line in lines:
                ...
                if match:
                    ...
                    for reason in reasons:
                        ...
                        ports_match = re.search(r'Port Scanning \((\d+) port[ei]\)', reason)
                        if ports_match:
    ...
    def parse_analyze(cls, output: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Parse top sources
            sources_section = re.search(r'TOP \d+ IP SORGENTE(.*?)(?=TOP \d+ PORTE|$)', clean_output, re.DOTALL)
            if sources_section:
                for line in sources_section.group(1).split('\n'):
                    source_match = re.search(r'(\d+\.\d+\.\d+\.\d+)\s*:\s*(\d+)\s+pacchetti\s+\((\d+(?:\.\d+)?)%\)', line)
                    if source_match:
            ...
            # Parse top ports
            ports_section = re.search(r'TOP \d+ PORTE DESTINAZIONE(.*?)(?=PROTOCOLLI|$)', clean_output, re.DOTALL)
            if ports_section:
                for line in ports_section.group(1).split('\n'):
                    port_match = re.search(r'(\d+)\s+\((\w+)\)\s*:\s*(\d+)\s+pacchetti', line)
                    if port_match:
            ...
            if proto_section:
                for line in proto_section.group(1).split('\n'):
                    # TCP: 1100 (89.1%)
                    proto_match = re.search(r'(TCP|UDP|ICMP):\s*(\d+)\s+\((\d+(?:\.\d+)?)%\)', line, re.IGNORECASE)
                    if proto_match:

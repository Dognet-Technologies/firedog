...
class TargetAdmin(admin.ModelAdmin):
    list_display = ['ip_address', 'hostname', 'status', 'firedog_version', 'last_seen', 'created_at']
    ...
    fieldsets = (
        ('Identificazione', {
            'fields': ('ip_address', 'hostname', 'description')
        }),
        ('Stato', {
            'fields': ('status', 'firedog_version', 'error_message')
        }),
        ('Configurazione SSH', {
            'fields': ('ssh_port', 'ssh_user')
        }),
        ('Metadata', {

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('DRY RUN MODE - Nessun file verrà scaricato'))
    ...
    def pull_target_status(self, target, dry_run=False) -> bool:
        ...
        if hasattr(target, 'group') and target.group:
            local_dir = Path(settings.FIREDOG_DATA_DIR) / 'groups' / target.group.name / hostname
        else:
            local_dir = Path(settings.FIREDOG_DATA_DIR) / 'groups' / 'default' / hostname
        ...
        if dry_run:
            self.stdout.write(f'    SCP: {target.ssh_user}@{target.ip_address}:{remote_path} → {local_path}')
            return True
        ...
        try:
            ...
            exit_code, stdout, stderr = ssh.execute_command(f'test -f {remote_path} && echo "EXISTS"')

            if 'EXISTS' not in stdout:
                self.stdout.write(self.style.WARNING(f'    ⚠ File remoto non trovato: {remote_path}'))
                ssh.disconnect()
                return False
            ...
            logger.info(f'Pull successful for target {target.id} ({hostname}): {file_size} bytes')

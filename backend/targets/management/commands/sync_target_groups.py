...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('DRY RUN MODE - Nessuna modifica verrà applicata'))

        # Trova tutti i target con un gruppo assegnato
        targets_with_gruppo = Target.objects.exclude(gruppo__isnull=True).exclude(gruppo='')

        self.stdout.write(f'Trovati {targets_with_gruppo.count()} target con un gruppo assegnato')
        ...
        for target in targets_with_gruppo:
            ...
            if target.gruppo == 'custom':
                ...
                group_name = dict(Target.GRUPPO_CHOICES).get(target.gruppo, target.gruppo)

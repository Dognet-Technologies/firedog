...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('DRY RUN MODE - Nessun dato verrà salvato nel DB'))

        # Base directory
        data_dir = Path(settings.FIREDOG_DATA_DIR)

        if not data_dir.exists():
            self.stdout.write(self.style.ERROR(f'Directory dati non trovata: {data_dir}'))
            return
        ...
        for target in targets:
            try:
                result = self.import_target_status(target, data_dir, dry_run, skip_rules, skip_threats)
                if result == 'success':
                    success_count += 1
                elif result == 'skipped':
                    skipped_count += 1
    ...
    def import_target_status(self, target, data_dir, dry_run=False, skip_rules=False, skip_threats=False):
        """
        ...
        if hasattr(target, 'group') and target.group:
            possible_paths.insert(0, data_dir / 'groups' / target.group.name / hostname / 'status.json')
        ...
        try:
            ...
            if not self.validate_json_data(data):
                self.stdout.write(self.style.ERROR(f'    ✗ JSON non valido o non sicuro'))
                return 'error'

            if dry_run:
                self.stdout.write(f'    [DRY RUN] JSON validato: {len(data.get("rules", {}).get("INPUT", []))} INPUT rules')
                return 'success'
            ...
            with transaction.atomic():
                ...
                if FirewallStats.objects.filter(target=target, collected_at=collected_at).exists():
                    self.stdout.write(self.style.WARNING(f'    ⊘ Già importato: {collected_at}'))
                    return 'skipped'
                ...
                logger.info(f'Imported target {target.id} status: {rules_count} rules, {threats_count} threats')
    ...
    def validate_json_data(self, data):
        ...
        total_rules = sum(len(rules.get(chain, [])) for chain in ['INPUT', 'OUTPUT', 'FORWARD'])
    ...
    def import_stats(self, target, data, collected_at):
        ...
        stats = FirewallStats.objects.create(
            ...
            pcap_input_dropped_bytes=self.sanitize_int(pcap_sizes.get('input_dropped_bytes', 0)),
            pcap_output_dropped_bytes=self.sanitize_int(pcap_sizes.get('output_dropped_bytes', 0)),
        ...
        for threat_data in threats_data:
            try:
                ...
                threat, created = ThreatLog.objects.get_or_create(
                    ...
                    detected_at__gte=timezone.now() - timezone.timedelta(hours=1),  # Evita duplicati recenti
                    defaults={

...
class SSHTerminalConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_connect(self, message):
        ...
        ssh_password = message.get(
            "password"
        )  # Password SSH per fallback autenticazione
        ...
        try:
            ...
            shell_started = await sync_to_async(self.ssh_manager.start_shell)(
    ...
    async def handle_install_firedog(self, message):
        ...
        try:
            ...
            await self.send_output(
                "\x1b[1;36m║\x1b[0m    \x1b[1;33mFireDog Installation Wizard\x1b[0m                        \x1b[1;36m║\x1b[0m\r\n"
            )
            await self.send_output(
                "\x1b[1;36m╚══════════════════════════════════════════════════════════╝\x1b[0m\r\n\r\n"
            )
            await self.send_output(
                f"Target: \x1b[1m{self.target.hostname or self.target.ip_address}\x1b[0m ({self.target.ip_address})\r\n"
            )
            ...
            if ssh_password:
                await self.send_output(
                    f"Auth: \x1b[1;33mPassword\x1b[0m (prima installazione)\r\n\r\n"
                )
            else:
            ...
            await self.send_output(
                "\x1b[1;34m[1/8]\x1b[0m Connessione SSH al target...\r\n"
            )
            ...
            try:
                ...
                await self.send_output(
                    "\x1b[32m  ✓ Connessione SSH stabilita\x1b[0m\r\n\r\n"
                )
            except Exception as e:
            ...
            if ssh_password:
                ...
                await self.send_output(
                    "\x1b[33m🔑 Configurando autenticazione con chiave pubblica per futuri accessi...\x1b[0m\r\n"
                )
                ...
                if key_configured:
                    await self.send_output(
                        "\x1b[32m  ✓ Chiave SSH configurata con successo\x1b[0m\r\n"
                    )
                    await self.send_output(
                        "\x1b[32m  ✓ Prossime connessioni useranno la chiave pubblica\x1b[0m\r\n\r\n"
                    )
                else:
                    await self.send_output(
                        "\x1b[33m  ⚠ Configurazione chiave SSH fallita, continuo con password\x1b[0m\r\n\r\n"
                    )
            else:
                await self.send_output(
            ...
            await self.send_output(
                "\x1b[1;34m[3/8]\x1b[0m Preparazione installazione...\r\n"
            )
            await self.send_output(
                "\x1b[33m💡 L'installazione richiederà la tua password sudo. Preparati a inserirla quando richiesto.\x1b[0m\r\n\r\n"
            )
            ...
            try:
                ...
                await sync_to_async(sftp_ssh.execute_command)(
                    f"mkdir -p {package_remote_path}"
                )

                # Upload package
                success, message = await sync_to_async(sftp_ssh.upload_directory)(
                ...
                await self.send_output(
                    "\x1b[32m  ✓ Pacchetto caricato con successo\x1b[0m\r\n\r\n"
                )
            except Exception as e:
            ...
            await self.send_output(
                "\x1b[1;34m[5/8]\x1b[0m Configurazione permessi...\r\n"
            )
            ...
            await self.send_output(
                "\x1b[1;34m[6/8]\x1b[0m Preparazione script di installazione...\r\n"
            )
            await self.send_output(
                "\x1b[33m⚠️  ATTENZIONE: Tra poco verrà richiesta la password sudo\x1b[0m\r\n"
            )
            await self.send_output(
                "\x1b[33m⚠️  La password NON verrà visualizzata mentre la digiti (è normale)\x1b[0m\r\n\r\n"
            )
            await asyncio.sleep(2.0)

            # STEP 7: Esecuzione install.sh INTERATTIVO
            await self.send_output(
                "\x1b[1;34m[7/8]\x1b[0m Esecuzione script di installazione...\x1b[0m\r\n"
            )
            ...
            await self.send_output(
                "\x1b[33m>>> Se richiesto, inserisci la password sudo e premi INVIO <<<\x1b[0m\r\n\r\n"
            )
            ...
            logger.info(
                f"Installation script started for target {target_id} - waiting for user interaction"
            )
    ...
    def update_target_status(
        self, status: str, installation_status: str = None, version: str = None
    ):
    ...
    async def verify_installation_result(self):
        ...
        try:
            ...
            if exit_code == 0 and "NOT_INSTALLED" not in stdout:
                ...
                await self.update_target_status(
                    "online", "Installation completed", version
                )
                logger.info(
                    f"Installation verified successfully for target {self.target.id}: v{version}"
                )
            else:
                # Installazione non completata o fallita
                await self.update_target_status(
                    "error", "Installation incomplete or failed"
                )
                logger.warning(
                    f"Installation verification failed for target {self.target.id}"
                )

        except Exception as e:
            logger.exception(
                f"Error verifying installation for target {self.target.id}: {e}"
            )
    ...
    async def handle_resize(self, message):
        ...
        try:
            await sync_to_async(self.ssh_manager.resize_terminal)(width, height)
    ...
    async def handle_disconnect(self):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "disconnected", "message": "Sessione terminata"}
            )
        )
    ...
    async def read_ssh_output(self):
        ...
        try:
            while self.is_connected and self.ssh_manager:
                ...
                data = await sync_to_async(self.ssh_manager.receive_data)(timeout=0.1)

                if data:
                    # Invia output al client
                    await self.send(
                        text_data=json.dumps({"type": "output", "data": data})
                    )
